...
const Calificaciones = () => {
  ...
  const mergeDataWithLabels = (labels, data) => {
    ...
    // Crear mapa de datos recibidos del backend
    data.forEach(item => {
      const formattedPeriod = formatPeriod(item.periodo, timeFilter);
      dataMap.set(formattedPeriod, {
        avgRating: parseFloat(item.avgRating) || 0,
        count: parseInt(item.count) || 0
      });
    });
    
    // Mapear cada label con los datos correspondientes
    return labels.map(label => ({
      period: label,
      avgRating: dataMap.get(label)?.avgRating || 0,
      count: dataMap.get(label)?.count || 0
    }));
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setSummary({
          totalRatings: parseInt(summaryResponse.data.totalRatings) || 0,
          avgRating: parseFloat(summaryResponse.data.avgRating || 0).toFixed(2),
          excellenceRate: parseFloat(summaryResponse.data.excellenceRate || 0).toFixed(1)
        });
      } catch (error) {
  ...
  const formatPeriod = (periodo, filter) => {
    if (filter === 'dia') {
      // Para días, el periodo viene como número de hora
      return `${periodo.toString().padStart(2, '0')}:00`;
    }
    
    // Para otros períodos, asegurar que tengamos una fecha válida
    const date = new Date(periodo);
    if (isNaN(date.getTime())) {
      console.warn('Fecha inválida recibida:', periodo);
      return periodo;
    }
